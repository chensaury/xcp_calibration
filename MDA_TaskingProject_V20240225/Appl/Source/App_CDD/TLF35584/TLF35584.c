...
void initTLF35584(tlf35584 *tlfDevice)
{
    ...
    //60us
    volatile int TLF35584_cnt = 0;
    for(int i = 0; i < 600; i++){
    	TLF35584_cnt ++;
    	if(TLF35584_cnt > 10000)
    		TLF35584_cnt = 0;
    }
...
uint16 transferDataTLF35584(spiCommandType cmd, tlf35584RegAddr addr, uint8 data)
{
    ...
    uint16 TLF35584_TxBuffer[1];
    uint16 TLF35584_RxBuffer[1];
    ...
    TLF35584_TxBuffer[0] = spiFrame.U;

    /* Send a data stream through the SPI Master */
    Std_ReturnType stdRet = E_NOT_OK;

    stdRet = Spi_SetupEB(8, TLF35584_TxBuffer, TLF35584_RxBuffer, 1);

    if( E_OK == stdRet )
    {
        stdRet = Spi_SyncTransmit(8);
    }
    return(TLF35584_RxBuffer[0]);
}
...
uint8 getMONSF0StatusFlagsTFL35584(tlf35584 *tlfDevice)
{
    tlf35584SpiFrameType spiFrame;

    spiFrame.U = transferDataTLF35584(SpiCommand_read, MONSF0Addr, (uint8)DUMMY_DATA);

    return spiFrame.B.data;
}

uint8 getMONSF1StatusFlagsTFL35584(tlf35584 *tlfDevice)
{
    tlf35584SpiFrameType spiFrame;

    spiFrame.U = transferDataTLF35584(SpiCommand_read, MONSF1Addr, (uint8)DUMMY_DATA);

    return spiFrame.B.data;
}

uint8 getMONSF2StatusFlagsTFL35584(tlf35584 *tlfDevice)
{
    tlf35584SpiFrameType spiFrame;

    spiFrame.U = transferDataTLF35584(SpiCommand_read, MONSF2Addr, (uint8)DUMMY_DATA);

    return spiFrame.B.data;
}

uint8 getMONSF3StatusFlagsTFL35584(tlf35584 *tlfDevice)
{
    tlf35584SpiFrameType spiFrame;

    spiFrame.U = transferDataTLF35584(SpiCommand_read, MONSF3Addr, (uint8)DUMMY_DATA);

    return spiFrame.B.data;
}

uint8 getOTFAILStatusFlagsTFL35584(tlf35584 *tlfDevice)
{
    tlf35584SpiFrameType spiFrame;

    spiFrame.U = transferDataTLF35584(SpiCommand_read, OTFAILAddr, (uint8)DUMMY_DATA);
