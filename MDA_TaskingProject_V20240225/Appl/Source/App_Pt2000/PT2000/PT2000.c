...
const unsigned short MAX_SPI_MODE_A_TRANSFER_SIZE = 31;  // max size for register config transfer

#define PT2000_Enable      TRUE
#define PT2000_Disable     FALSE
...
bool DRVEN_check(void)
{
    ...
    Dio_WriteChannel(DioConf_DioChannel_V_DRIVEN1,PT2000_Disable);
    send_single_PT2000_SPI_Cmd(WRITE, selection_register, 0x100); // Select common page
...
bool BIST_check(_Bool MBIST_run, _Bool LBIST_run)
{
    ...
    if(MBIST_run ==1)
    {
     ...
     //   delay100us(40); // wait 4 ms until MBIST complete
        ...
        while (rxData == 0x1)
        {
            rxData = send_single_PT2000_SPI_Cmd(READ, main_BIST_interface, 1);
        //    delay100us(1);
        }
...
//////////////////////////////////////////////////////////////////////////////////////////////////
unsigned short Read_VbatADC (void)
{
...
bool ID_Check (void)
{
    ...
    if (rxData != 0xAE21) { ID_results = false; }
...
//////////////////////////////////////////////////////////////////////////////////////////////////
unsigned long Bootstrap_check (void)
{
    unsigned short rxData = 0;
...
// Return type     : void
//////////////////////////////////////////////////////////////////////////////////////////////////
void Tracer (unsigned short trace_start, unsigned short trace_stop, _Bool ucore, unsigned char channel , unsigned char post_trigger_length, _Bool trace_enable)
{
...
else
{
    ...
    for (k = 0; k < size; k++)            // downloads RAM
    {
    	sendBuf[2*k]  = ((PT2000_data_RAM [k]) >> 8) & 0xFF;
    	sendBuf[(2*k+1)]  = (PT2000_data_RAM [k])& 0xFF;
    }
...
// peak涓嬭浇鍑芥暟
void download_Peak_RAM(int p_target)
{
    unsigned short memory_area = 0;        // memory area
    unsigned short start_address = 0;      // start address
    unsigned short codeWidthRegAddr = 0;   // code width register address
    ...
    case CODE_RAM1:
        ...
        start_address = 0;
        codeWidthRegAddr = ch1_code_width;
        RAM_ptr = PT2000_code_Peak_RAM1;
        size = sizeof(PT2000_code_Peak_RAM1) / 2;   // gets number of words to be sent
        break;
    ...
    case CODE_RAM2:
        ...
        start_address = 0;
        codeWidthRegAddr = ch2_code_width;
        RAM_ptr = PT2000_code_Peak_RAM2;
        size = sizeof(PT2000_code_Peak_RAM2) / 2;   // gets number of words to be sent
        break;
    ...
    case CODE_RAM3:
        ...
        start_address = 0;
        codeWidthRegAddr = ch3_code_width;
        RAM_ptr = PT2000_code_Peak_RAM3;
        size = sizeof(PT2000_code_Peak_RAM3) / 2;   // gets number of words to be sent
        break;
...
void download_Peak_register(int r_target)
{
    int n = 0;                           // used for loop for writing data to the chip
    unsigned short r_start_address = 0;  // start address
    unsigned short r_size = 0;           // size of configuration data
    ...
    case CH1_REG:                                // channel 1 configurations
        r_start_address = ch1_flash_enable;
        reg_ptr = PT2000_ch1_Peak_config;
        r_size = sizeof(PT2000_ch1_Peak_config) / 2;  // number of words to be sent
        break;

    case CH2_REG:                                // channel 2 configurations
        r_start_address = ch2_flash_enable;
        reg_ptr = PT2000_ch2_Peak_config;
        r_size = sizeof(PT2000_ch2_Peak_config) / 2;  // number of words to be sent
        break;

    case CH3_REG:                                // channel 3 configurations
        r_start_address = ch3_flash_enable;
        reg_ptr = PT2000_ch3_Peak_config;
        r_size = sizeof(PT2000_ch3_Peak_config) / 2;  // number of words to be sent
        break;

    case DIAG_REG:                               // diagnostic configurations
        r_start_address = diag_ls1_diag_config1;
        reg_ptr = PT2000_diag_Peak_config;
        r_size = sizeof(PT2000_diag_Peak_config) / 2; // number of words to be sent
        break;

    case IO_REG:                                 // IO configurations
        r_start_address = io_fbk_sens_uc0_ch1_part1;
        reg_ptr = PT2000_io_Peak_config;
        r_size = sizeof(PT2000_io_Peak_config) / 2;   // number of words to be sent
        break;

    case MAIN_REG:                               // main configurations
        r_start_address = main_Clock_Prescaler;
        reg_ptr = PT2000_main_Peak_config;
        r_size = sizeof(PT2000_main_Peak_config) / 2; // number of words to be sent
        break;
    ...
    case CH1_REG:
        // Program the ch1 flash enable register
        send_single_PT2000_SPI_Cmd(WRITE, ch1_flash_enable, PT2000_ch1_Peak_config[0]);
        break;

    case CH2_REG:
        // Program the ch2 flash enable register
        send_single_PT2000_SPI_Cmd(WRITE, ch2_flash_enable, PT2000_ch2_Peak_config[0]);
        break;

    case CH3_REG:
        // Program the ch3 flash enable register
        send_single_PT2000_SPI_Cmd(WRITE, ch3_flash_enable, PT2000_ch3_Peak_config[0]);
        break;
...
//  涓�閫氶亾 鎭掓祦 2 閫氶亾鎭掓祦 3閫氶亾 PEAK_HOLD
void download_Constant1_Constant2_Peak3_RAM(int target)
{
    unsigned short memory_area = 0;        // memory area
    unsigned short start_address = 0;      // start address
    unsigned short codeWidthRegAddr = 0;   // code width register address
...
void download_Constant1_Constant2_Peak3_register(int r_target)
{
    int n = 0;                           // used for loop for writing data to the chip
    unsigned short r_start_address = 0;  // start address
    unsigned short r_size = 0;           // size of configuration data
...
//  涓�閫氶亾鎭掓祦   2 閫氶亾PEAK_HOLD    3閫氶亾鎭掓祦
void download_Constant1_Peak2_Constant3_RAM(int target)
{
    unsigned short memory_area = 0;        // memory area
    unsigned short start_address = 0;      // start address
    unsigned short codeWidthRegAddr = 0;   // code width register address
    ...
    case CODE_RAM1:
        ...
        start_address = 0;
        codeWidthRegAddr = ch1_code_width;
        RAM_ptr = PT2000_code_Constant1_Peak2_RAM1;
        size = sizeof(PT2000_code_Constant1_Peak2_RAM1) / 2;   // gets number of words to be sent
        break;
...
void download_Constant1_Peak2_Constant3_register(int r_target)
{
    int n = 0;                           // used for loop for writing data to the chip
    unsigned short r_start_address = 0;  // start address
    unsigned short r_size = 0;           // size of configuration data
    ...
    case CH1_REG:                                // channel 1 configurations
        r_start_address = ch1_flash_enable;
        reg_ptr = PT2000_ch1_Constant1_Peak2_config;
        r_size = sizeof(PT2000_ch1_Constant1_Peak2_config) / 2;  // number of words to be sent
        break;
    ...
    case CH1_REG:
        // Program the ch1 flash enable register
        send_single_PT2000_SPI_Cmd(WRITE, ch1_flash_enable, PT2000_ch1_Constant1_Peak2_config[0]);
        break;
...
//  涓�閫氶亾鎭掓祦   2 閫氶亾PEAK_HOLD    3閫氶亾PEAK_HOLD
void download_Constant1_Peak2_Peak3_RAM(int target)
{
    unsigned short memory_area = 0;        // memory area
    unsigned short start_address = 0;      // start address
    unsigned short codeWidthRegAddr = 0;   // code width register address
...
void download_Constant1_Peak2_Peak3_register(int r_target)
{
    int n = 0;                           // used for loop for writing data to the chip
    unsigned short r_start_address = 0;  // start address
    unsigned short r_size = 0;           // size of configuration data
...
//  涓�閫氶亾PEAK_HOLD   2 閫氶亾鎭掓祦    3閫氶亾鎭掓祦
void download_Peak1_Constant2_Constant3_RAM(int target)
{
    unsigned short memory_area = 0;        // memory area
    unsigned short start_address = 0;      // start address
    unsigned short codeWidthRegAddr = 0;   // code width register address
    ...
    case CODE_RAM1:
        ...
        start_address = 0;
        codeWidthRegAddr = ch1_code_width;
        RAM_ptr =  PT2000_code_Peak1_Constant2_RAM1;
        size = sizeof( PT2000_code_Peak1_Constant2_RAM1) / 2;   // gets number of words to be sent
        break;
...
void download_Peak1_Constant2_Constant3_register(int r_target)
{
    int n = 0;                           // used for loop for writing data to the chip
    unsigned short r_start_address = 0;  // start address
    unsigned short r_size = 0;           // size of configuration data
    ...
    case CH1_REG:                                // channel 1 configurations
        r_start_address = ch1_flash_enable;
        reg_ptr = PT2000_ch1_Peak1_Constant2_config;
        r_size = sizeof(PT2000_ch1_Peak1_Constant2_config) / 2;  // number of words to be sent
        break;
    ...
    case CH1_REG:
        // Program the ch1 flash enable register
        send_single_PT2000_SPI_Cmd(WRITE, ch1_flash_enable, PT2000_ch1_Peak1_Constant2_config[0]);
        break;
...
//  涓�閫氶亾PEAK_HOLD   2 閫氶亾鎭掓祦    3閫氶亾Peak_Hold
void download_Peak1_Constant2_Peak3_RAM(int target)
{
    unsigned short memory_area = 0;        // memory area
    unsigned short start_address = 0;      // start address
    unsigned short codeWidthRegAddr = 0;   // code width register address
...
void download_Peak1_Constant2_Peak3_register(int r_target)
{
    int n = 0;                           // used for loop for writing data to the chip
    unsigned short r_start_address = 0;  // start address
    unsigned short r_size = 0;           // size of configuration data
...
//  涓�閫氶亾PEAK_HOLD   2 閫氶亾Peak_Hold    3閫氶亾鎭掓祦
void download_Peak1_Peak2_Constant3_RAM(int target)
{
    unsigned short memory_area = 0;        // memory area
    unsigned short start_address = 0;      // start address
    unsigned short codeWidthRegAddr = 0;   // code width register address
...
void download_Peak1_Peak2_Constant3_register(int r_target)
{
    int n = 0;                           // used for loop for writing data to the chip
    unsigned short r_start_address = 0;  // start address
    unsigned short r_size = 0;           // size of configuration data
...
// 涓嬭浇鏁翠綋鏁版嵁浣� 鈥斺�� PEAK_HOLD
void ProgramDevice_Peak(void)
{
    download_Peak_RAM(CODE_RAM1);        // transfers code RAM1
    download_Peak_RAM(CODE_RAM2);        // transfers code RAM2
    download_Peak_RAM(CODE_RAM3);        // transfers code RAM3
    download_Peak_RAM(DATA_RAM);         // transfers data R
    download_Peak_register(MAIN_REG);         // download main register configurations
    download_Peak_register(CH1_REG);          // download channel 1 register configurations
    download_Peak_register(CH2_REG);          // download channel 2 register configurations
    download_Peak_register(CH3_REG);          // download channel 3 register configurations
    download_Peak_register(IO_REG);           // download IO register configurations
    download_Peak_register(DIAG_REG);         // download diag register configurations
}

// 涓嬭浇鏁翠綋鏁版嵁浣� 鈥斺�� PEAK_HOLD
void ProgramDevice_Constant1_Constant2_Peak3(void)
{
    download_Constant1_Constant2_Peak3_RAM(CODE_RAM1);        // transfers code RAM1
    download_Constant1_Constant2_Peak3_RAM(CODE_RAM2);        // transfers code RAM2
    download_Constant1_Constant2_Peak3_RAM(CODE_RAM3);        // transfers code RAM3
    download_Constant1_Constant2_Peak3_RAM(DATA_RAM);         // transfers data R
    download_Constant1_Constant2_Peak3_register(MAIN_REG);         // download main register configurations
    download_Constant1_Constant2_Peak3_register(CH1_REG);          // download channel 1 register configurations
    download_Constant1_Constant2_Peak3_register(CH2_REG);          // download channel 2 register configurations
    download_Constant1_Constant2_Peak3_register(CH3_REG);          // download channel 3 register configurations
    download_Constant1_Constant2_Peak3_register(IO_REG);           // download IO register configurations
    download_Constant1_Constant2_Peak3_register(DIAG_REG);         // download diag register configurations
}

// 涓嬭浇鏁翠綋鏁版嵁浣� 鈥斺�� PEAK_HOLD
void ProgramDevice_Constant1_Peak2_Constant3(void)
{
    download_Constant1_Peak2_Constant3_RAM(CODE_RAM1);        // transfers code RAM1
    download_RAM(CODE_RAM2);        // transfers code RAM2        // transfers code RAM2
    download_Constant1_Peak2_Constant3_RAM(CODE_RAM3);        // transfers code RAM3
    download_Constant1_Peak2_Constant3_RAM(DATA_RAM);         // transfers data R
    download_Constant1_Peak2_Constant3_register(MAIN_REG);         // download main register configurations
    download_Constant1_Peak2_Constant3_register(CH1_REG);          // download channel 1 register configurations
    download_register(CH2_REG);     // download channel 2 register configurations        // download channel 2 register configurations
    download_Constant1_Peak2_Constant3_register(CH3_REG);          // download channel 3 register configurations
    download_Constant1_Peak2_Constant3_register(IO_REG);           // download IO register configurations
    download_Constant1_Peak2_Constant3_register(DIAG_REG);         // download diag register configurations
}

// 涓嬭浇鏁翠綋鏁版嵁浣� 鈥斺�� PEAK_HOLD
void ProgramDevice_Constant1_Peak2_Peak3(void)
{
    download_Constant1_Peak2_Peak3_RAM(CODE_RAM1);        // transfers code RAM1
    download_Constant1_Peak2_Peak3_RAM(CODE_RAM2);        // transfers code RAM2
    download_Constant1_Peak2_Peak3_RAM(CODE_RAM3);        // transfers code RAM3
    download_Constant1_Peak2_Peak3_RAM(DATA_RAM);         // transfers data R
    download_Constant1_Peak2_Peak3_register(MAIN_REG);         // download main register configurations
    download_Constant1_Peak2_Peak3_register(CH1_REG);          // download channel 1 register configurations
    download_Constant1_Peak2_Peak3_register(CH2_REG);          // download channel 2 register configurations
    download_Constant1_Peak2_Peak3_register(CH3_REG);          // download channel 3 register configurations
    download_Constant1_Peak2_Peak3_register(IO_REG);           // download IO register configurations
    download_Constant1_Peak2_Peak3_register(DIAG_REG);         // download diag register configurations
}

// 涓嬭浇鏁翠綋鏁版嵁浣� 鈥斺�� PEAK_HOLD
void ProgramDevice_Peak1_Constant2_Constant3(void)
{
    download_Peak1_Constant2_Constant3_RAM(CODE_RAM1);        // transfers code RAM1
    download_Peak1_Constant2_Constant3_RAM(CODE_RAM2);        // transfers code RAM2
    download_Peak1_Constant2_Constant3_RAM(CODE_RAM3);        // transfers code RAM3
    download_Peak1_Constant2_Constant3_RAM(DATA_RAM);         // transfers data R
    download_Peak1_Constant2_Constant3_register(MAIN_REG);         // download main register configurations
    download_Peak1_Constant2_Constant3_register(CH1_REG);          // download channel 1 register configurations
    download_Peak1_Constant2_Constant3_register(CH2_REG);          // download channel 2 register configurations
    download_Peak1_Constant2_Constant3_register(CH3_REG);          // download channel 3 register configurations
    download_Peak1_Constant2_Constant3_register(IO_REG);           // download IO register configurations
    download_Peak1_Constant2_Constant3_register(DIAG_REG);         // download diag register configurations
}

// 涓嬭浇鏁翠綋鏁版嵁浣� 鈥斺�� PEAK_HOLD
void ProgramDevice_Peak1_Constant2_Peak3(void)
{
    download_Peak1_Constant2_Peak3_RAM(CODE_RAM1);        // transfers code RAM1
    download_Peak1_Constant2_Peak3_RAM(CODE_RAM2);        // transfers code RAM2
    download_Peak1_Constant2_Peak3_RAM(CODE_RAM3);        // transfers code RAM3
    download_Peak1_Constant2_Peak3_RAM(DATA_RAM);         // transfers data R
    download_Peak1_Constant2_Constant3_register(MAIN_REG);         // download main register configurations
    download_Peak1_Constant2_Constant3_register(CH1_REG);          // download channel 1 register configurations
    download_Peak1_Constant2_Constant3_register(CH2_REG);          // download channel 2 register configurations
    download_Peak1_Constant2_Constant3_register(CH3_REG);          // download channel 3 register configurations
...
// 涓嬭浇鏁翠綋鏁版嵁浣� 鈥斺�� Peak_HOLD
void ProgramDevice_Peak1_Peak2_Constant3(void)
{
    download_Peak1_Peak2_Constant3_RAM(CODE_RAM1);        // transfers code RAM1
    download_Peak1_Peak2_Constant3_RAM(CODE_RAM2);        // transfers code RAM2
    download_Peak1_Peak2_Constant3_RAM(CODE_RAM3);        // transfers code RAM3
    download_Peak1_Peak2_Constant3_RAM(DATA_RAM);         // transfers data R
    download_Peak1_Peak2_Constant3_register(MAIN_REG);         // download main register configurations
    download_Peak1_Peak2_Constant3_register(CH1_REG);          // download channel 1 register configurations
    download_Peak1_Peak2_Constant3_register(CH2_REG);          // download channel 2 register configurations
    download_Peak1_Peak2_Constant3_register(CH3_REG);          // download channel 3 register configurations
    download_Peak1_Peak2_Constant3_register(IO_REG);           // download IO register configurations
    download_Peak1_Peak2_Constant3_register(DIAG_REG);         // download diag register configurations
}
