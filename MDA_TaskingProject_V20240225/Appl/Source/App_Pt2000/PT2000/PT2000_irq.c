...
extern uint16 rxData;
extern uint16 cksys_missing_error;
extern uint16 cksys_missing;
extern uint16 drv_irq;
extern uint16 checksum_irq;
extern uint16 drv_status;
extern uint16 spi_irq;
extern uint16 auto_irq;
extern uint16 sw_irq;
extern uint16 inj1_error;
extern uint16 inj2_error;
extern uint16 inj3_error;
extern uint16 inj4_error;
extern uint16 inj5_error;
extern uint16 inj6_error;
extern uint16 fp_error;
extern uint16 dcdc_error;
...
uint16 AutoErrStatus;
uint16 interrupt_reg1 = 0;
uint16 interrupt_reg2 = 0;



uint16 HS1_OPEN_OR_VBAT_SHORT_GND   = 0;
uint16 HS1_SHORT_BAT = 0 ;
uint16 HS2_VBOOST_SHORT_GND = 0;
uint16 HS2_OPEN = 0;
uint16 HS2_VBOOST_SHORT_DRAIN_SOURCE = 0;
uint16 LS2_OPEN_OR_LS2_SHORT_VBAT_OR_VBOOST = 0;
uint16 LS2_SHORT_GND = 0;


uint16 HS3_OPEN_OR_VBAT_SHORT_GND   = 0;
uint16 HS3_SHORT_BAT = 0 ;
uint16 HS4_VBOOST_SHORT_GND = 0;
uint16 HS4_OPEN = 0;
uint16 HS4_VBOOST_SHORT_DRAIN_SOURCE = 0;
uint16 LS4_OPEN_OR_LS4_SHORT_VBAT_OR_VBOOST = 0;
uint16 LS4_SHORT_GND  = 0;


uint16 HS5_OPEN_OR_VBAT_SHORT_GND   = 0;
uint16 HS5_SHORT_BAT = 0 ;
uint16 HS6_VBOOST_SHORT_GND = 0;
uint16 HS6_OPEN = 0;
uint16 HS6_VBOOST_SHORT_DRAIN_SOURCE = 0;
uint16 LS6_OPEN_OR_LS6_SHORT_VBAT_OR_VBOOST = 0;
uint16 LS6_SHORT_GND  = 0;


uint16 HS7_OPEN_OR_VBAT_SHORT_GND   = 0;
uint16 HS7_SHORT_BAT = 0 ;
uint16 LS7_OPEN_OR_LS7_SHORT_VBAT_OR_VBOOST = 0;
uint16 LS7_SHORT_GND  = 0;
...
//uint8 ProcessPT2000Interrupts_CSIndex_Control()
//{
//
//    for (uint8 i = 1;i <= 6;i++)
//    {
//        Set_CSPin_State(i,OFF);
//
...
void irq_arithmetic()
{
    ...
    HS1 = HS1_OPEN_OR_VBAT_SHORT_GND + HS1_SHORT_BAT << 1 ;
    HS2 = HS2_VBOOST_SHORT_GND + HS2_OPEN  << 1  + HS2_VBOOST_SHORT_DRAIN_SOURCE  <<  2  ;
    LS2 = LS2_OPEN_OR_LS2_SHORT_VBAT_OR_VBOOST + LS2_SHORT_GND  << 1;



    HS3 = HS3_OPEN_OR_VBAT_SHORT_GND + HS3_SHORT_BAT << 1 ;
    HS4 = HS4_VBOOST_SHORT_GND + HS4_OPEN  << 1  + HS4_VBOOST_SHORT_DRAIN_SOURCE  <<  2  ;
    LS4 = LS4_OPEN_OR_LS4_SHORT_VBAT_OR_VBOOST + LS4_SHORT_GND  << 1;



    HS5 = HS5_OPEN_OR_VBAT_SHORT_GND + HS5_SHORT_BAT << 1 ;
    HS6 = HS6_VBOOST_SHORT_GND + HS6_OPEN  << 1  + HS6_VBOOST_SHORT_DRAIN_SOURCE  <<  2  ;
    LS6 = LS6_OPEN_OR_LS6_SHORT_VBAT_OR_VBOOST + LS6_SHORT_GND << 1 ;


    HS7 = HS7_OPEN_OR_VBAT_SHORT_GND + HS7_SHORT_BAT << 1 ;
    LS7 = LS7_OPEN_OR_LS7_SHORT_VBAT_OR_VBOOST + LS7_SHORT_GND  <<  1 ;
...
void ProcessAutomaticInterrupts()
{
    ...
    if ((auto_irq & 0x1) == 1)  // Error occurred on uc0ch1
    {
        ...
        if ((status & 0x2) == 2)   // INJ2 fails b0 at 1
        {
            ...
            if (AutoErrStatus & 0x8 != 0)
            {
                HS1_OPEN_OR_VBAT_SHORT_GND   = 1;
            }

            if (AutoErrStatus & 0x10 != 0)
            {
            ...
            if (AutoErrStatus & 0x20 != 0)
            {
                HS2_VBOOST_SHORT_GND = 1;
            }

            if (AutoErrStatus & 0x40 != 0)
            {
                HS2_OPEN = 1;
            }

            if (AutoErrStatus & 0x80 != 0)
            {
                HS2_VBOOST_SHORT_DRAIN_SOURCE = 1;
            }

            if (AutoErrStatus & 0x100 != 0)
            {

                LS2_OPEN_OR_LS2_SHORT_VBAT_OR_VBOOST = 1;

            }

            if (AutoErrStatus & 0x200 != 0)
            {
    ...
    if ((auto_irq & 0x2) == 1)  // Error occurred on uc1ch1
    {
        ...
        if ((status & 0x2) == 2)   // INJ4 fails b0 at 1
        {
            ...
            if (AutoErrStatus & 0x8 != 0)
                       ...
                       if (AutoErrStatus & 0x80 != 0)
                       {
                            HS4_VBOOST_SHORT_DRAIN_SOURCE = 1;
                       }

                       if (AutoErrStatus & 0x100 != 0)
                       {

                            LS4_OPEN_OR_LS4_SHORT_VBAT_OR_VBOOST = 1;
    ...
    if ((auto_irq & 0x4) == 1)  // Error occurred on uc0ch2
    {
        ...
        if ((status & 0x2) == 2)   // INJ6 fails b0 at 1
        {
            ...
            if (AutoErrStatus & 0x80 != 0)
            {
                HS6_VBOOST_SHORT_DRAIN_SOURCE = 1;
            }

            if (AutoErrStatus & 0x100 != 0)
            {

                LS6_OPEN_OR_LS6_SHORT_VBAT_OR_VBOOST = 1;
    ...
    if ((auto_irq & 0x10) == 1)  // Error occurred on uc0ch3
    {
        ...
        if ((status & 0x1) == 1)                                             // FP fails b0 at 1
        {
            ...
            if (AutoErrStatus & 0x100 != 0)
            {
                ...
                LS7_OPEN_OR_LS7_SHORT_VBAT_OR_VBOOST = 1;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
uint16 AutoErr(unsigned char INJ)
{
    ...
    if ((HSxData & 0x4) == 0x4)     // HSx was ON when fault occurred
    {
        if ((HSxData & 0x1) != 0)   // VDS should be 0 if no error
            {
                HS_Bat_error = 1;
                if ((HSxData & 0x2) == 0) Fault_signature = High_side_VBAT_OPEN_OR_VBAT_SHORT_GND; // HS gate open or HS disconnected
    ...
    else        // HSx was OFF when fault occurred
    {
        ...
        {
            HS_Bat_error = 1;
            Fault_signature = HSX_SHORT_VBAT;
        }
    }
        ...
        if ((HSxData & 0x20) == 0x20)         // HSx was ON when fault occurred
            {
                ...
                {
                    HS_Boost_error = 1;
                    if ((HSxData & 0x10) != 0) Fault_signature = High_side_VBoost_OPEN; // MOSOPEN or shorted to VBAT(but this should be detected in idle)
                    else Fault_signature = VBOOST_SHORT_GND;                        // HS shorted to GND
                }
            }
        else        // HSx was OFF when fault occurred
            {
                ...
                {
                    HS_Boost_error = 1;
                    Fault_signature = VBOOST_SHORT_DRAIN_SOURCE;  // This can also be gate HS Boost open
                }
            }
    }
    ...
    if ((LSxData & 0x2) == 0x2)  // LSx was ON when fault occurred
    {
        if ((LSxData & 0x1) != 0)
        {
            ...
            Fault_signature = Low_side_OPEN_OR_Low_side_SHORT_VBAT_OR_VBOOST;
        }
    }
