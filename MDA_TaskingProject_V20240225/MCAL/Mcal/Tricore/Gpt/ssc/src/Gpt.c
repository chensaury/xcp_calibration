...
#if ( GPT_AR_RELEASE_MINOR_VERSION != 2U )
...
#define GPT_ISR_FROM_CM0                ((uint8)0x1U)
...
/* MISRA2012_RULE_04_9_JUSTIFICATION: Function like macro defined for easy
...
#define GPT_GET_GTMTIMERINFO(COREID, CHANNEL)                                  \
                  ((&(GPT_GET_CORECONFIGPTR(COREID))->                         \
                                  ChannelConfigPtr[CHANNEL])->GptGtmTimerInfo)
...
LOCAL_INLINE void Gpt_lSetNormalMode (\
          const Gpt_CoreConfigType *LocalCoreConfigPtr, uint32 CurrentCoreId);
...
/* MISRA2012_RULE_5_5_JUSTIFICATION: External identifiers going beyond 32 chars.
  in generated code due to Autosar Naming constraints.*/
#define GPT_START_SEC_VAR_INIT_ASIL_B_GLOBAL_UNSPECIFIED
/* MISRA2012_RULE_20_1_JUSTIFICATION: Memmap header usage as per Autosar
   guideline. */
/* MISRA2012_RULE_4_10_JUSTIFICATION: Memmap header is repeatedly included
   without safeguard. It complies to Autosar guidelines. */
...
/* MISRA2012_RULE_5_5_JUSTIFICATION: External identifiers going beyond 32 chars.
  in generated code due to Autosar Naming constraints.*/
#define GPT_STOP_SEC_VAR_INIT_ASIL_B_GLOBAL_UNSPECIFIED
/* MISRA2012_RULE_20_1_JUSTIFICATION: Memmap header usage as per Autosar
   guideline. */
/* MISRA2012_RULE_4_10_JUSTIFICATION: Memmap header is repeatedly included
   without safeguard. It complies to Autosar guidelines. */
                                            ...
                                            (GPT_DEV_ERROR_DETECT == STD_ON)) */
  {
                                      ...
                                      Gpt_DriverMode == GPT_MODE_NORMAL)
    {
                                         ...
                                         GPT_ENABLE_NOTIF) == GPT_ENABLE_NOTIF)
      {
        ...
        Disable notification interrupt only if the wakeup flag is cleaed and
...
#if (GPT_INITCHECK_API == STD_ON)
/*******************************************************************************
** Traceability   : [cover parentID={86147709-85ED-4413-BB21-AC61123F0375}]   **
**                                                                            **
...
/* [cover parentID={B0FE8EE0-269F-48fd-9A23-01B581C9F807}]
...
[/cover] */
  ...
  [/cover] */
  IErrVal = Gpt_lMulCoreInitDetCheck(ConfigPtr, CurrentCoreId);
                                  ...
                                  Gpt_CoreConfigType const *const CoreConfigPtr)
{
            ...
            (Counter < CoreConfigPtr->Gpt_MaxNormalChannels);  Counter++ )
  {
    ...
    Initialize Gtm Channel and corresponding  status flag
    [/cover] */
                                  ...
                                  )
{
  ...
  else
  {
    ...
    /* [cover parentID={D457A09E-5E6E-452b-8BCD-9A0932114D5B}]
    Is Single Core?
    [/cover] */
    #else
    /* [cover parentID={B4B05C0A-6C47-4608-9884-13E73AB88CEE}]
   ...
   [/cover] */
    if (Gpt_lCheckTimeOutVaue(TimeOutVal, GPT_GTM_TOM_TIMER_MAX) == TRUE)
    {
      ...
      /* MISRA2012_RULE_18_4_JUSTIFICATION: Pointer arithmetic used due to
        PBConfigStructure and is within allowed range.*/
      TomChannelRegPtr = GTM_TOM_CH_POINTER(ModuleNumber, ChannelNumber);
      /* [cover parentID={89CA7543-F7ED-4b20-AA60-7F5C584FF96C}]
      ONESHOT Mode
      [/cover] */
      #if (GPT_ONESHOT_USED == STD_ON)
      /* [cover parentID={9B5B2177-A2B1-43a4-9BFA-6EA6746C0134}]
      ...
      /* [cover parentID={534DE206-AA1F-4bbf-895B-73660B8A3E54}]
      ONESHOT Mode
      [/cover] */
    #if (GPT_ONESHOT_USED == STD_ON)
      /* [cover parentID={363D1376-AAA1-4a3f-BC7B-590077E47308}]
      Is oneshot mode enabled?
      [/cover] */
      if (OneShotModeStatus == (boolean)TRUE)
      {
        /* Clear Notify */
        TomChannelRegPtr->IRQ.NOTIFY.U = GPT_CLEAR_INTERRUPTS;
        /* [cover parentID={1FCE24AE-0E51-49ae-9AFB-8E4F09852129}]
        Write CN0 again to start the channel
        [/cover] */
        /* Write CN0 again to start the channel */
        TomChannelRegPtr->CN0.U = (uint8)0x0U;
      }
      #endif
      /* End of #if (GPT_ONESHOT_USED == STD_ON)*/
                                      ...
                                      )
{
  ...
  /* [cover parentID={E3AC716B-645F-47bb-9114-402FAC67E13E}]
    ...
    RetVal = (uint32)((TomChannelCount2 * GPT_GTM_TOM_TIMER_MAX) + \
                                                  TomChannelCount1);
                                    ...
                                    )
{
  ...
  else
  {
    ...
    [/cover] */
    IErrVal = Gpt_lMulCoreChannelDetCheck(CurrentCoreId, Channel, Gpt_Sid);
    /* [cover parentID={30A3E95A-ADC4-4e9c-9388-B6CABBDAE49C}]
    Single Core
    [/cover] */
    #else
    /* [cover parentID={F4D828F0-FADB-4f60-ACAE-63E8C605DF6F}]
    Clear the error flag as No Error
    [/cover] */
    IErrVal = GPT_E_DET_NO_ERR;
    #endif
  }
...
#if (GPT_WAKEUP_FUNCTIONALITY_API == STD_ON)
/*******************************************************************************
** Traceability   : [cover parentID={4DBEAFF3-7E62-4212-BE3A-DA3946E123CA}]   **
**                                                                            **
                                  ...
                                  )
{
  ...
  */
  if (GPT_MODE_ONESHOT == GptChannelPtr->GptChannelMode)
  {
    ...
    Stops a TOM/ATOM channel in one shoot mode and reset CN0 reg value
    [/cover]
    */
    Gpt_lGtmHandleOsm(GptChannelPtr);

    /*
    Set the status to expired in case of Timer completion in One Shot Mode
                                      ...
                                      )
{
  ...
  if (CurrentCoreId == MCAL_MASTER_COREID)
  {
    /* [cover parentID={7FD9B0CD-8D87-4c73-83F8-7A265B061AE5}]
    Is ConfigPtr valid?
    [/cover] */
    if (ConfigPtr == NULL_PTR )
    {
      /* [cover parentID={60E01AEF-F0D2-41aa-A8CE-265E583D0975}]
      Report DET
      [/cover] */
      Gpt_lReportMulticoreError(GPT_SID_INIT, GPT_E_INIT_FAILED);

      IErrVal = GPT_E_DET_ERR;
    }
  }
  /* Check DET for Slave Core Init */
  else
  {
    /* [cover parentID={71C05B3D-D421-4aeb-872E-2A5C92C93D98}]
    Is Gpt configured in the current core?
    [/cover] */
    if (GPT_GET_CORECONFIGPTR(CurrentCoreId) == NULL_PTR)
    {
    ...
    [/cover] */
    else if (GPT_DRIVER_INITIALIZED != \
            Mcal_GetBitAtomic(Gpt_DriverStatus, BitPos, GPT_CORE_STATUS_LEN))
    {
...
void Gpt_Isr(const Gpt_ChannelType LogicalChId,const uint32 StatusFlags)
{
  ...
  Gpt_ChannelType             LocalChannel;
       ...
       ((GPT_WAKEUP_FUNCTIONALITY_API == STD_ON)        && \
       (GPT_REPORT_WAKEUP_SOURCE == STD_ON)))
  /* Check if the ISR is valid for GPT (triggered by CM0) */
  if (StatusFlags == GPT_ISR_FROM_CM0)
  {
      /* Get the current core ID */
      CurrentCoreId = GPT_GET_COREID();
