...
/* Covering MemMaps for const data as there is no const data in ICU */
/* [/cover] */
...
#if ( ICU_17_TIMERIP_AR_RELEASE_MINOR_VERSION != 2U )
...
#if ( ICU_17_TIMERIP_SW_MAJOR_VERSION != 3U )
...
/* Root variable list */
static Icu_17_TimerIp_CoreType* Icu_17_TimerIp_Varlist[ICU_17_TIMERIP_MAX_CORES];
/* MISRA2012_RULE_5_1_JUSTIFICATION: External identifiers going beyond 32 chars.
  in generated code due to Autosar Naming constraints.*/
/* MISRA2012_RULE_5_2_JUSTIFICATION: External identifiers going beyond 32 chars.
  in generated code due to Autosar Naming constraints.*/
...
LOCAL_INLINE void Icu_lCommonVariableInit(void);

#if (ICU_17_TIMERIP_INITCHECK_API == STD_ON)
LOCAL_INLINE Std_ReturnType Icu_lCommonVariableInitCheck(void);
#endif
...
#if (ICU_17_TIMERIP_TIMESTAMP_API == STD_ON)
LOCAL_INLINE void Icu_lGtmTimeStamp_Isr(
    const uint32 CoreId,
    const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr,
    const uint32 *const GlbIndexStatusPtr);
#endif
    ...
    const Icu_17_TimerIp_ConfigType *const ConfigPtr)
{
...
#endif
  {
    ...
    /* Common initializations for global variables.*/
    /* [cover parentID={3088CBB1-84BA-43b7-B6F9-A455EB6F65A2}]
    Is Master Core?
    [/cover] */
    if(CoreId == MCAL_MASTER_COREID)
    {
      /* [cover parentID={B31F013E-575C-4724-9384-E224B5AC9F9C}]
      Initialize shared global variables which are shared among cores.
      [/cover] */
      /* Global Variable Initializations */
      Icu_lCommonVariableInit();
#if (ICU_17_TIMERIP_GPT12_USED == STD_ON)
      /* [cover parentID={A2BBA217-688E-4147-AA5D-78037A570A98}]
      Initialize GPT12 clock selection.
      [/cover] */
      if(Icu_17_TimerIp_ConfigPtr->Gpt1TimerClockSel != ICU_TIMER_UNUSED)
      {
        /* Initialize GPT1 Timer clock selection */
        /* [cover parentID={12FC5EBB-18F3-48f8-8E39-68ED1E259CCC}]
        Set the GPT1 clock selection
        [/cover] */
        Mcu_17_Gpt12_TimerClockInit(
            MCU_GPT12_GPT1_BLOCK,
            Icu_17_TimerIp_ConfigPtr->Gpt1TimerClockSel);
      }
      if(Icu_17_TimerIp_ConfigPtr->Gpt2TimerClockSel != ICU_TIMER_UNUSED)
      {
        /* Initialize GPT2 Timer clock selection */
        /* [cover parentID={B97619F7-69C5-4a65-AB24-907B587CCFC0}]
        Set GPT2 clock selection.
        [/cover] */
        Mcu_17_Gpt12_TimerClockInit(
            MCU_GPT12_GPT2_BLOCK,
            Icu_17_TimerIp_ConfigPtr->Gpt2TimerClockSel);
      }
#endif
    }
...
void Icu_17_TimerIp_EnableEdgeCount(const Icu_17_TimerIp_ChannelType Channel)
{
     ...
     * (ICU_17_TIMERIP_SAFETY_ENABLE == STD_ON) */
  {
    ...
    /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
    ...
    (ICU_17_TIMERIP_EDGE_COUNT_API == STD_ON)
            ...
            /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
               ...
               other than array indexing used is being used. no side effect
               has been seen*/
            EdgeCountValuePtr = (Icu_17_TimerIp_Varlist[CoreId]-> \
                ChannelDataBasePtr) + ChIndex;
...
void Icu_17_TimerIp_ResetEdgeCount(const Icu_17_TimerIp_ChannelType Channel)
{
     ...
     * (ICU_17_TIMERIP_SAFETY_ENABLE == STD_ON) */
  {
    ...
    /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
       ...
       other than array indexing used is being used. no side effect
       has been seen*/
    EdgeCountValuePtr = (Icu_17_TimerIp_Varlist[CoreId]->ChannelDataBasePtr)\
        + ChIndex;
    /* Depending on ICU option take action */
    IcuOption = (uint32)LocalIcuChanPtr->IcuProperties.AssignedHwUnit;
    switch(IcuOption)
...
#else
        /* Although this is never practically possible, this is required 
           to avoid a compiler warning.*/
        RetVal =  (Icu_17_TimerIp_EdgeNumberType)0;
#endif
        break;
      default:
        /* [cover parentID={D79FFF30-406A-4d47-AE82-DB0FB2B627A9}]
        If GPT12 edge counter is used?
        [/cover] */
        /* Call function Icu_lGpt12GetEdgeNumbers to get edge numbers */
#if (ICU_17_TIMERIP_GPT12_USED == STD_ON) && \
    (ICU_17_TIMERIP_EDGE_COUNT_API == STD_ON)
    ...
    const Icu_17_TimerIp_ChannelType Channel)
{
  ...
  /* #if  (ICU_17_TIMERIP_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
     ...
     other than array indexing used is being used. no side effect
     has been seen*/
    GlbIndexStatusPtr = (Icu_17_TimerIp_Varlist[CoreId]->GlbIndexStatusBasePtr)\
        + CoreChIndex;
            ...
            == (uint32) ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT)
        {
          ...
          if (IcuOption == ICU_17_TIMERIP_GPT12_OPTION)
          {
            ...
            /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
             ...
             other than array indexing used is being used. no side effect
             has been seen*/
            ChnlDataPtr = (Icu_17_TimerIp_Varlist[CoreId]->ChannelDataBasePtr) \
                + ChIndex;
...
/* STATIC CHECK: if DET or Safety error reporting is enabled */
  ...
  ErrorFlag = Icu_lNotifiDetCheck(Channel,ICU_17_TIMERIP_SID_ENABLEWAKEUP,
                ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT,CoreId);
    ...
    (ICU_17_TIMERIP_SIGNAL_MEASUREMENT_API == STD_ON)
        ...
        default:
          ...
          /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
             ...
             other than array indexing used is being used. no side effect
             has been seen*/
          SigMeasurePtr = Icu_17_TimerIp_Varlist[CoreId]->ChannelDataBasePtr\
              + ChIndex;
    ...
    const Icu_17_TimerIp_ActivationType  Activation)
{
  ...
  ErrorFlag = Icu_lSetActivationConditionDETCheck(CoreId,Channel);
    if (ErrorFlag != ICU_ERROR_FOUND)
    {
      ...
      [/cover] */
      if (Activation > ICU_17_TIMERIP_BOTH_EDGES)
      {
   ...
   * (ICU_17_TIMERIP_SAFETY_ENABLE == STD_ON)*/
  {
    ...
    Is channel mode SIGNAL MEASURE?
    [/cover] */
    if (LocalIcuChanPtr->IcuProperties.MeasurementMode
        == (uint32) ICU_17_TIMERIP_MODE_SIGNAL_MEASUREMENT)
    {
...
/* #if (ICU_17_TIMERIP_DEV_ERROR_DETECT == STD_ON) ||
    ...
    Is Activity Started on any channel?
    [/cover] */
    ...
    else
    {
      ...
      switch (IcuHwOption)
      {
        ...
        default:
          ...
          Set the GPT12 register for the new edge type
          [/cover] */
          Icu_lGpt12SetActivationCondition(CellUsed, Activation);
#endif
          break;
      }
        ...
        if (ErrorFlag != ICU_ERROR_FOUND)
        {
              ...
              Channel = Channel + 1U)
          {
            ...
            /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
             ...
             other than array indexing used is being used. no side effect
             has been seen*/
            GlbIndexStatusPtr = GlbIndexStatusPtr + 1U;
            /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
             associated.to make it easier to understand, pointer arithmetic
             other than array indexing used is being used. no side effect
             has been seen*/
            LocalIcuChanPtr = LocalIcuChanPtr + 1U;
    ...
    ||(ICU_17_TIMERIP_ERU_SIGEDGE_USED == STD_ON)
        ...
        switch (IcuOption)
        {
          ...
          case ICU_17_TIMERIP_CCU_OPTION:
            ...
            TimerId = ((uint32)CellUsed & ICU_CCU_GET_LOWER_NIBBLE);
            TimerId |= (((uint32)CellUsed & ICU_CCU_GET_UPPER_NIBBLE) << 
    ...
    const uint32 EdgeCount)
{
  ...
  if (ICU_ERROR_FOUND != ErrorFlag)
  {
    ...
    CoreChIndex = (uint8)Channel;
#endif
    /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are associated.
       to make it easier to understand, pointer arithmetic other than array
       indexing used is being used. no side effect has been seen*/
    LocalIcuChanPtr = \
        (Icu_17_TimerIp_ConfigPtr->CoreConfig[CoreId]->ChannelConfigPtr)\
        + CoreChIndex;
...
Safety Properties
[/cover]*/
/* CYCLOMATIC_Icu_17_TimerIp_InitCheck_JUSTIFICATION: This Function has 
cyclomatic complexity of 17 as it has to check for multiple global variables 
and master core specific gloabal checks. Function cannot be broken down to 
smaller blocks as it will impact maintanability and will complicate the 
implementation.*/
    ...
    const Icu_17_TimerIp_ConfigType* const ConfigPtr)
{
  ...
  /* [cover parentID={5908A9DE-ABD5-4b24-882E-8726DEF89514}]
  Is ErrorFlag E_OK and Core Id is master Core Id?
  [/cover] */
    if((ErrorFlag ==E_OK) && (CoreId == MCAL_MASTER_COREID))
    {
      /* [cover parentID={EBE4971D-5F2B-4c67-9295-15352FA795B2}]
      Master Core Specific Checks
      [/cover] */
      /* [cover parentID={1A99EE2B-50A0-422c-A42E-63F0B48F5AF6}]
         Check if the Global Ram is  Initialized for all used channel
      [/cover] */
     ErrorFlag = Icu_lCommonVariableInitCheck();
     /* [cover parentID={EA49BE1A-9DCF-4ad8-87D8-D96CA0944B3D}]
     Is GPT12 used?
     [/cover] */
#if (ICU_17_TIMERIP_GPT12_USED == STD_ON)
     if(ErrorFlag ==E_OK)
     {
        if(Icu_17_TimerIp_ConfigPtr->Gpt1TimerClockSel != ICU_TIMER_UNUSED)
        {
          /* Check GPT1 Timer clock selection */
          /* [cover parentID={0550B8DB-C838-4471-8309-32C1FB7455C5}]
          Read clock prescaler for GPT1 block and verify against the configured 
          value.
          [/cover] */
          CmpVal &= ~(Icu_17_TimerIp_ConfigPtr->Gpt1TimerClockSel ^ 
                            Mcal_GetBitAtomic(MODULE_GPT120.T3CON.U,
                            IFX_GPT12_T3CON_BPS1_OFF,IFX_GPT12_T3CON_BPS1_LEN));
        }
        if(Icu_17_TimerIp_ConfigPtr->Gpt2TimerClockSel != ICU_TIMER_UNUSED)
        {
          /* Check GPT2 Timer clock selection */
          /* [cover parentID={D8AD48AF-273C-4f84-AB8E-0D8D96AA7CE0}]
          Read clock prescaler for GPT2 block and verify against the configured 
          value.
          [/cover] */
          CmpVal &= ~(Icu_17_TimerIp_ConfigPtr->Gpt2TimerClockSel ^ 
                            Mcal_GetBitAtomic(MODULE_GPT120.T6CON.U,
                            IFX_GPT12_T6CON_BPS2_OFF,IFX_GPT12_T6CON_BPS2_LEN));
        }
 ...
 * (ICU_17_TIMERIP_SAFETY_ENABLE == STD_ON) */
/*******************************************************************************
** Traceability: [cover parentID= {BFA87920-C227-40b9-86CB-E27E6AE782F1}]     **
**                                                                            **
** Syntax :           LOCAL_INLINE void Icu_lCommonVariableInit(void)         **
**                                                                            **
** Description :      Local function :Service to initialize all the required  **
**                    global variables for all the cores.                     **
** [/cover]                                                                   **
**                                                                            **
** Service ID:        None                                                    **
**                                                                            **
...
LOCAL_INLINE void Icu_lCommonVariableInit(void)
{
  ...
  Icu_17_TimerIp_Varlist[0] = &Icu_17_TimerIp_VarlistCore0;
  /* Channel flags base pointer for Core 0*/
  Icu_17_TimerIp_VarlistCore0.GlbIndexStatusBasePtr =
      Icu_17_TimerIp_GlbIndexStatusCore0;
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE0 > 0U)
  /* Data Channel parameter base address for Core 0*/
  Icu_17_TimerIp_VarlistCore0.ChannelDataBasePtr =
  ...
  Icu_17_TimerIp_Varlist[1] = &Icu_17_TimerIp_VarlistCore1;
  /* Channel flags base pointer for Core 1*/
  Icu_17_TimerIp_VarlistCore1.GlbIndexStatusBasePtr =
      Icu_17_TimerIp_GlbIndexStatusCore1;
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE1 > 0U)
  /* Data Channel parameter base address for Core 1*/
  Icu_17_TimerIp_VarlistCore1.ChannelDataBasePtr =
  ...
  Icu_17_TimerIp_Varlist[2] = &Icu_17_TimerIp_VarlistCore2;
  /* Channel flags base pointer for Core 2*/
  Icu_17_TimerIp_VarlistCore2.GlbIndexStatusBasePtr =
      Icu_17_TimerIp_GlbIndexStatusCore2;
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE2 > 0U)
  /* Data Channel parameter base address for Core 2*/
  Icu_17_TimerIp_VarlistCore2.ChannelDataBasePtr =
  ...
  Icu_17_TimerIp_Varlist[3] = &Icu_17_TimerIp_VarlistCore3;
  /* Channel flags base pointer for Core 3*/
  Icu_17_TimerIp_VarlistCore3.GlbIndexStatusBasePtr =
      Icu_17_TimerIp_GlbIndexStatusCore3;
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE3 > 0U)
  /* Data Channel parameter base address for Core 3*/
  Icu_17_TimerIp_VarlistCore3.ChannelDataBasePtr =
  ...
  Icu_17_TimerIp_Varlist[4] = &Icu_17_TimerIp_VarlistCore4;
  /* Channel flags base pointer for Core 4*/
  Icu_17_TimerIp_VarlistCore4.GlbIndexStatusBasePtr =
      Icu_17_TimerIp_GlbIndexStatusCore4;
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE4 > 0U)
  /* Data Channel parameter base address for Core 4*/
  Icu_17_TimerIp_VarlistCore4.ChannelDataBasePtr =
  ...
  Icu_17_TimerIp_Varlist[5] = &Icu_17_TimerIp_VarlistCore5;
  /* Channel flags base pointer for Core 5*/
  Icu_17_TimerIp_VarlistCore5.GlbIndexStatusBasePtr =
      Icu_17_TimerIp_GlbIndexStatusCore5;
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE5 > 0U)
  /* Data Channel parameter base address for Core 5*/
  Icu_17_TimerIp_VarlistCore5.ChannelDataBasePtr =
      ...
      Channel++)
  {
    switch ((uint32) LocalIcuChanPtr->IcuProperties.MeasurementMode)
    {
      case (uint32) ICU_17_TIMERIP_MODE_SIGNAL_MEASUREMENT:
        ChannelDataPtr->SigMeasureParasCcu6.ActiveTime = 0U;
        ChannelDataPtr->SigMeasureParasCcu6.FirstEdgeFlag = 0U;
        ChannelDataPtr->SigMeasureParasCcu6.RegValue = 0U;
        break;
      ...
      case (uint32) ICU_17_TIMERIP_MODE_TIMESTAMP:
        ...
        ChannelDataPtr->TimeCaptureParas.Notify = 0U;
        ChannelDataPtr->TimeCaptureParas.NotifyCounter = 0U;
        ChannelDataPtr->TimeCaptureParas.Size = 0U;
        break;
      default: /* ICU_17_TIMERIP_MODE_EDGE_COUNTER */
        ChannelDataPtr->EdgeCountParas.EdgeCounter = 0U;
        break;
    }
    /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
     associated.to make it easier to understand, pointer arithmetic
    ...
    /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
     ...
     other than array indexing used is being used. no side effect
     has been seen*/
    ChannelDataPtr = ChannelDataPtr + 1U;
  }
...
/*******************************************************************************
** Traceability: [cover parentID={8311B858-BBB8-4aab-AAF1-01243FA552D2}]      **
**                                                                            **
** Syntax :           LOCAL_INLINE Std_ReturnType                             **
**                                     Icu_lCommonVariableInitCheck(void)     **
**                                                                            **
** Description :      Local function :Service to Check all required           **
**                    global for all the cores are initialized .              **
** [/cover]                                                                   **
**                                                                            **
** Service ID:        None                                                    **
**                                                                            **
...
** Parameters (out):  None                                                    **
**                                                                            **
** Return value:      Error Status flag                                       **
**                                                                            **
*******************************************************************************/
/* CYCLOMATIC_Icu_lCommonVariableInitCheck_JUSTIFICATION: This Function has 
cyclomatic complexity of 23 as it has to check for each core's configuration 
pointer and corresponding elements. Hence the function cannot be broken down
further. There is no loss in readability and maintainability with this
implementation.*/
LOCAL_INLINE Std_ReturnType Icu_lCommonVariableInitCheck(void)
{
  Std_ReturnType ErrorFlag;
  uint32 CmpVal = ICU_INITCHECK_NO_ERR_VAL;

  ErrorFlag = E_NOT_OK;

  /* Check for global Ram variable init for Core 0 */
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE0 > 0U)
  /* Check for Variable list root pointer for Core 0*/
  if (Icu_17_TimerIp_Varlist[0] == &Icu_17_TimerIp_VarlistCore0)
  {
    /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
     integer type due to global variable access. */
    /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
     unsigned long type due to global variable access. */
    CmpVal &= ~((uint32) (Icu_17_TimerIp_VarlistCore0.GlbIndexStatusBasePtr) \
        ^ (uint32) Icu_17_TimerIp_GlbIndexStatusCore0);
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE0 > 0U)
    /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
     integer type due to global variable access. */
    /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
     unsigned long type due to global variable access. */
    CmpVal &= ~((uint32) (Icu_17_TimerIp_VarlistCore0.ChannelDataBasePtr) \
        ^ (uint32) Icu_17_TimerIp_ChannelDataCore0);
#endif
  }
#endif
  /* Check for global Ram variable init for Core 1 */
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE1 > 0U)
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE0 > 0U)
  if (CmpVal == ICU_INITCHECK_NO_ERR_VAL)
#endif
  {
    /* Check for Variable list root pointer for Core 1*/
    if (Icu_17_TimerIp_Varlist[1] == &Icu_17_TimerIp_VarlistCore1)
    {
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore1.GlbIndexStatusBasePtr) \
          ^(uint32)Icu_17_TimerIp_GlbIndexStatusCore1);
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE1 > 0U)
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore1.ChannelDataBasePtr) \
          ^(uint32)Icu_17_TimerIp_ChannelDataCore1);
#endif
    }
  }
#endif

  /* Check for global Ram variable init for Core 2 */
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE2 > 0U)
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE0 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE1 > 0U)
  if (CmpVal == ICU_INITCHECK_NO_ERR_VAL)
#endif
  {
    /* Check for Variable list root pointer for Core 2*/
    if (Icu_17_TimerIp_Varlist[2] == &Icu_17_TimerIp_VarlistCore2)
    {
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore2.GlbIndexStatusBasePtr) \
          ^(uint32)Icu_17_TimerIp_GlbIndexStatusCore2);
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE2 > 0U)
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore2.ChannelDataBasePtr) \
          ^(uint32)Icu_17_TimerIp_ChannelDataCore2);
#endif
    }
  }
#endif

  /* Check for global Ram variable init for Core 3 */
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE3 > 0U)
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE0 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE1 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE2 > 0U)
  if (CmpVal == ICU_INITCHECK_NO_ERR_VAL)
#endif
  {
    /* Check for Variable list root pointer for Core 3*/
    if (Icu_17_TimerIp_Varlist[3] == &Icu_17_TimerIp_VarlistCore3)
    {
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore3.GlbIndexStatusBasePtr) \
          ^(uint32)Icu_17_TimerIp_GlbIndexStatusCore3);
      ...
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore3.ChannelDataBasePtr) \
          ^(uint32)Icu_17_TimerIp_ChannelDataCore3);
#endif
    }
  }
#endif

  /* Check for global Ram variable init for Core 4 */
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE4 > 0U)
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE0 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE1 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE2 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE3 > 0U)
  if (CmpVal == ICU_INITCHECK_NO_ERR_VAL)
#endif
  {
    /* Check for Variable list root pointer for Core 4*/
    if (Icu_17_TimerIp_Varlist[4] == &Icu_17_TimerIp_VarlistCore4)
    {
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore4.GlbIndexStatusBasePtr) \
          ^ (uint32)Icu_17_TimerIp_GlbIndexStatusCore4);
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE4 > 0U)
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore4.ChannelDataBasePtr) \
          ^(uint32)Icu_17_TimerIp_ChannelDataCore4);
#endif
    }
  }
#endif

  /* Check for global Ram variable init for Core 5 */
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE5 > 0U)
#if (ICU_17_TIMERIP_MAX_CHANNELS_CORE0 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE1 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE2 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE3 > 0U) || \
    (ICU_17_TIMERIP_MAX_CHANNELS_CORE4 > 0U)
  if (CmpVal == ICU_INITCHECK_NO_ERR_VAL)
#endif
  {
    /* Check for Variable list root pointer for Core 5*/
    if (Icu_17_TimerIp_Varlist[5] == &Icu_17_TimerIp_VarlistCore5)
    {
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore5.GlbIndexStatusBasePtr) \
          ^(uint32)Icu_17_TimerIp_GlbIndexStatusCore5);
#if (ICU_17_TIMERIP_CHNL_DATA_MAX_CHANNELS_CORE5 > 0U)
      /* MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between a pointer and
       integer type due to global variable access. */
      /* MISRA2012_RULE_11_6_JUSTIFICATION: Conversion between a pointer and
       unsigned long type due to global variable access. */
      CmpVal &= ~((uint32)(Icu_17_TimerIp_VarlistCore5.ChannelDataBasePtr) \
          ^(uint32)Icu_17_TimerIp_ChannelDataCore5);
#endif
    }
  }
#endif
          ...
          && (ErrorFlag == E_OK); Channel++)
  {
    ...
    switch ((uint32) LocalIcuChanPtr->IcuProperties.MeasurementMode)
    {
      case (uint32) ICU_17_TIMERIP_MODE_SIGNAL_MEASUREMENT:
        ...
        CheckValue32 = ChannelDataPtr->SigMeasureParasCcu6.ActiveTime;
        CheckValue32 = CheckValue32 | \
          ChannelDataPtr->SigMeasureParasCcu6.FirstEdgeFlag;
        CheckValue32 = CheckValue32 | \
            ChannelDataPtr->SigMeasureParasCcu6.RegValue;
        if(CheckValue32 != 0U)
        {
          ErrorFlag = E_NOT_OK;
        }
      ...
      case (uint32) ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT:
        ...
        if (ChannelDataPtr->EdgeDetectParas.EdgeDetectCount != 0U)
        {
          ErrorFlag = E_NOT_OK;
        }
        break;
      case (uint32) ICU_17_TIMERIP_MODE_TIMESTAMP:
        CheckValue16 = ChannelDataPtr->TimeCaptureParas.Counter;
        CheckValue16 = CheckValue16 | ChannelDataPtr->TimeCaptureParas.Notify;
        CheckValue16 = CheckValue16 | \
            ChannelDataPtr->TimeCaptureParas.NotifyCounter;
        CheckValue16 = CheckValue16 | ChannelDataPtr->TimeCaptureParas.Size;
        if ((CheckValue16 != 0U) \
            && (ChannelDataPtr->TimeCaptureParas.Address != \
                (Icu_17_TimerIp_ValueType *)0U))
        {
        ...
        break;
      default: /* ICU_17_TIMERIP_MODE_EDGE_COUNTER */
        if(ChannelDataPtr->EdgeCountParas.EdgeCounter != 0U)
        {
          ErrorFlag = E_NOT_OK;
        }
        break;
    ...
    const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  Ccu6CfgPtr.TimerId = ((uint32)CellUsed & ICU_CCU_GET_LOWER_NIBBLE);
  Ccu6CfgPtr.TimerId |= (((uint32)CellUsed & ICU_CCU_GET_UPPER_NIBBLE)<< 
                            (ICU_MCU_CCU_COMPARATOR_POS - ICU_CCU_NIBBLE_SIZE));
  ...
  ErrorFlag= Mcu_17_Ccu6_TimerInitCheck(&Ccu6CfgPtr);
    ...
    Ccu6RegPtr = &(&MODULE_CCU60)[CellUsed & ICU_CCU_GET_LOWER_NIBBLE];
    ...
    const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  if(ChannelNo == (uint8)ICU_GPT_TIMER_T3)
  {
    Gpt12Ptr.TimerCtrlReg |= \
        (uint32)(Icu_17_TimerIp_ConfigPtr->Gpt1TimerClockSel) << \
            IFX_GPT12_T3CON_BPS1_OFF;
  }
  else if(ChannelNo == (uint8)ICU_GPT_TIMER_T6)
  {
    Gpt12Ptr.TimerCtrlReg |= \
        ((uint32)(Icu_17_TimerIp_ConfigPtr->Gpt2TimerClockSel) << \
            IFX_GPT12_T6CON_BPS2_OFF);
  }
  else
  {
    /* No clock initialization for rest of channels. */
  }
    ...
    const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
 ...
 /* Get the Eicr register to be Checked */
  SfrVal = (MODULE_SCU.EICR[(HwUsed & ICU_ERU_CHNL_MASK)/ICU_ERU_CHNL_PAIR_COUNT].U);
    ...
    const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  Gpt12Ptr.TimerCtrlReg |= ((uint32)ICU_GPT12_COUNTER_MODE) | LActiveEdge;
  if(ChannelNo == MCU_GPT12_TIMER3)
  {
    Gpt12Ptr.TimerCtrlReg |= \
        (uint32)(Icu_17_TimerIp_ConfigPtr->Gpt1TimerClockSel) << \
            IFX_GPT12_T3CON_BPS1_OFF;
  }
  else if(ChannelNo == MCU_GPT12_TIMER6)
  {
    Gpt12Ptr.TimerCtrlReg |= \
        ((uint32)(Icu_17_TimerIp_ConfigPtr->Gpt2TimerClockSel) << \
            IFX_GPT12_T6CON_BPS2_OFF);
    ...
    const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  Ptr = &(MODULE_SCU.EICR[(HwUsed & ICU_ERU_CHNL_MASK)/ICU_ERU_CHNL_PAIR_COUNT]);
    ...
    const uint8 CoreId)
{
  ...
  CoreChannelIndex = \
      ...
      & (ICU_CHNL_MAP_CHNL_INDEX_MASK);
#else
  CoreChannelIndex = (uint8)Channel;
#endif
  ...
  Is ICU measurement mode sleep and no error?
  [/cover] */
  ...
  Icu_17_TimerIp_ChannelData *const EdgeCountValuePtr)
{
  ...
  [/cover] */
  EdgeCountValuePtr->EdgeCountParas.EdgeCounter = 0U- (TimChannelRegPtr->CNT.U);
  /* MISRA2012_RULE_8_13_JUSTIFICATION: No side effects foreseen
  ...
  Icu_17_TimerIp_ChannelData *const EdgeCountValuePtr)
{
  ...
  /* MISRA2012_RULE_18_4_JUSTIFICATION: GPT12 T2 register is taken as a base
   * address and array indexing is performed to access Tx. The CellUsed is
   * extracted from generated configuration structure which ensures that it is
   * within bounds. */
    ...
    const uint16 GlobalChannelNumber)
{
  ...
  /* MISRA2012_RULE_18_4_JUSTIFICATION: GPT12 T2CON register is taken as a base
   * address and array indexing is performed to access TxCON. The CellUsed is
   * extracted from generated configuration structure which ensures that it is
   * within bounds. */
...
LOCAL_INLINE uint32 Icu_lStartTimestampDetCheck(
 ...
 {
  uint32 ErrorFlag = ICU_ERROR_NOT_FOUND;
  /* [cover parentID={5A79AAB3-F22B-4ad1-B417-4C79ED3C7E16}]
  ...
  if (BufferPtr == (Icu_17_TimerIp_ValueType*) NULL_PTR)
  {
    ...
    Mcal_ReportSafetyError(
        ...
        ICU_17_TIMERIP_INSTANCE_ID,
        ICU_17_TIMERIP_SID_STARTTIMESTAMP,
        ICU_17_TIMERIP_E_PARAM_POINTER);/* End of report to Safety */
  }
#else
  UNUSED_PARAMETER(BufferPtr);
#endif
    ...
    const uint8 CoreId)
{
  ...
  if (ErrorFlag != ICU_ERROR_FOUND)
  {
    ...
    if (ErrorFlag != ICU_ERROR_FOUND)
    {
      /* Initialize ICU configured channel ROM pointer */
#if (ICU_17_TIMERIP_SINGLE_CORE == STD_OFF)
      CoreChIndex = (Icu_17_TimerIp_ConfigPtr->ChannelMapping[Channel] &
          ICU_CHNL_MAP_CHNL_INDEX_MASK);
#else
      CoreChIndex = (uint8) Channel;
#endif
    ...
    const uint16 HwUnit)
{
  ...
  case ICU_17_TIMERIP_ERU_OPTION:
    ...
    [/cover] */
#if(ICU_17_TIMERIP_ERU_SIGEDGE_USED == STD_ON)
    OguOutput = (uint8)((HwUnit >> ICU_ERU_OGU_CHNL_BIT_POS) & ICU_ERU_CHNL_MASK);
...
LOCAL_INLINE void Icu_lGtmStartEdgeDetect(const uint16 GlobalChannelNumber)
{
  ...
  TimChannelRegPtr->CTRL.U &= (~((uint32)ICU_ERU_CHNL_MASK << ICU_GTM_TIM_MODE_POS));
    ...
    const uint8 CfgEdge)
{
  ...
  [/cover] */
  Mcu_17_Ccu6_TimerShadowTransfer(((uint32)CellUsed & ICU_CCU_GET_LOWER_NIBBLE)
      | (((uint32)CellUsed & ICU_CCU_GET_UPPER_NIBBLE) << 
      (ICU_MCU_CCU_COMPARATOR_POS - ICU_CCU_NIBBLE_SIZE)));
    ...
    uint32 *const GblIndexStatusPtr)
{
  ...
  CcChannelNumber =
      ((uint8)CellUsed & ICU_CCU_GET_UPPER_NIBBLE) >> ICU_BRING_TO_BIT_0;
  ...
  if(CcuRRegVal != (uint16)0U)
  {
    ...
    if(0U != CcuCmpstatValue)
    {
      ...
      [/cover] */
      /* Clear ST Flag(HW) */
      Ccu6RegPtr->CMPMODIF.U = CcuCmpstatValue <<ICU_CCU6_CMPMODIF_CLEAR_OFFSET;
    ...
    [/cover] */
        ...
        Mcal_GetBitAtomic(*GblIndexStatusPtr,ICU_SIGNALMESURE_INPUT_STAT_POS,
  ...
  if (ErrorFlag != ICU_ERROR_FOUND)
  {
    ...
    CoreChIndex = (uint8)Channel;
      ...
      )
      {
        ...
        Report error to indicate that the channel is invalid.
        [/cover] */
        Icu_lReportError(
    ...
    const uint8 Property)
{
  ...
  [/cover] */
  if((1U == CmpStatValue) &&
    ((((uint8)ICU_17_TIMERIP_MODE_SIGNAL_MEASUREMENT == Mode) &&
    ...
    const uint8 Mode)
{
  ...
  /* [cover parentID={695D764E-7EA1-4c9c-BAD8-8E9571628B8C}]
   ...
   [/cover] */
  if (((Mode == (uint8) ICU_17_TIMERIP_MODE_NORMAL) && (1U == NotifStatus)) \
      || ((Mode == (uint8) ICU_17_TIMERIP_MODE_SLEEP) && (1U == WakeupStatus)))
  {
    ...
    const Icu_17_TimerIp_ActivationType ActiveEdge)
{
  ...
  LActiveEdge = 0U;
  /* [cover parentID={57349094-0CC8-418f-AD94-B41A24E49862}]
  Is active edge valid?
  [/cover] */
  if(ActiveEdge < ICU_17_TIMERIP_NO_EDGE)
  {
          ...
          ICU_CHNL_WAKEUP_CAPABLE_POS, ICU_CHNL_WAKEUP_BIT_LENGTH))
  {
    ...
    else
    {
      ...
      if (IcuHwOption == ICU_17_TIMERIP_GPT12_OPTION)
      {
        ...
        /* Retrieve the multi edge Count value address */
        /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU
         * Channels are associated.to make it easier to
         * understand, pointer arithmetic other than array
         * indexing used is being used. no side effect has been
         * seen*/
        MultiEdgeCountPtr =\
            (Icu_17_TimerIp_Varlist[CoreId]->ChannelDataBasePtr)\
                + ChIndex;
          ...
          ICU_CHNL_WAKEUP_CAPABLE_POS, ICU_CHNL_WAKEUP_BIT_LENGTH))
   {
    ...
    [/cover] */
          ...
          ((uint8)ICU_17_TIMERIP_ACTIVE == ActiveStatus))
      {
        ...
        ChIndex = IcuChanPtr->ModeMappingIndex;
        /* Retrieve the multi edge Count value address */
        /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels
         *  are associated.to make it easier to understand, pointer
         *  arithmetic other than array indexing used is being used.
         *  no side effect has been seen*/
        MultiEdgeCountPtr =\
            (Icu_17_TimerIp_Varlist[CoreId]->ChannelDataBasePtr)\
                + ChIndex;
        /* Call GPT12 Start Edge Detect*/
    ...
    const uint8 CoreId)
{
  ...
  If Muticore error detection of safety is enabled
  [/cover] */
#if ((ICU_17_TIMERIP_MULTICORE_ERROR_DETECT == STD_ON) || \
    (ICU_17_TIMERIP_SAFETY_ENABLE == STD_ON))
...
/* end report safety */
    ...
    CoreChIndex = (uint8)Channel;
      ...
      /* Initialize ICU configured channel ROM pointer */
      /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
         associated to make it easier to understand, pointer arithmetic other
         than array indexing used is being used. no side effect has been seen */
      LocalIcuChanPtr = \
          (Icu_17_TimerIp_ConfigPtr->CoreConfig[CoreId]->ChannelConfigPtr) \
          + CoreChIndex ;
      ...
      switch (LocalIcuChanPtr->IcuProperties.AssignedHwUnit)
      {
        ...
        case ICU_17_TIMERIP_GTM_OPTION:
          ...
          [/cover] */
#if(ICU_17_TIMERIP_GTM_TIM_USED == STD_ON)
          Icu_lTim_Isr((Icu_17_TimerIp_ChannelType)Channel,StatusFlags);
#endif
          break;
        ...
        case ICU_17_TIMERIP_CCU_OPTION:
          ...
          [/cover] */
#if(ICU_17_TIMERIP_CCU6_USED == STD_ON)
          Icu_lCcu_Isr((Icu_17_TimerIp_ChannelType)Channel,StatusFlags);
#endif
          break;
          /* ERU ISR handler */
        ...
        case ICU_17_TIMERIP_ERU_OPTION:
          ...
          [/cover] */
#if(ICU_17_TIMERIP_ERU_SIGEDGE_USED == STD_ON)
          Icu_lEru_Isr((Icu_17_TimerIp_ChannelType)Channel);
#endif
          break;
        default:
          ...
          Call GPT12 ISR
          [/cover] */
          Icu_lGpt12_Isr((Icu_17_TimerIp_ChannelType)Channel);
#endif
          break;
      }
    }
...
/* End of report to Safety */
    ...
    case (uint32) ICU_17_TIMERIP_MODE_TIMESTAMP:
      ...
      if (NewvalSts == ICU_GTM_NEWVAL_FLAG_SET)
      {
        ...
        Timestamp for Gtm
        [/cover] */
        Icu_lGtmTimeStamp_Isr(CoreId, LocalIcuChanPtr, GlbIndexStatusPtr);
...
** Reentrancy:       Reentrant for different channel                          **
**                                                                            **
** Parameters (in):  Channel - Logical Channel Identifier has triggered       **
**                   the interrupt                                            **
    ...
    const Icu_17_TimerIp_ChannelType Channel)
{
  ...
  /* MISRA2012_RULE_18_4_JUSTIFICATION: Different ICU Channels are
    associated.to make it easier to understand, pointer arithmetic
    other than array indexing is being used. no side effect
    has been seen*/
  GlbIndexStatusPtr = (Icu_17_TimerIp_Varlist[CoreId]->GlbIndexStatusBasePtr) \
      + CoreChIndex;
#endif   
...
const Icu_17_TimerIp_ChannelType Channel)
{
  ...
  /* Extract the core channel ID wit respect to ERU Channel map */
#if (ICU_17_TIMERIP_SINGLE_CORE == STD_OFF)
  CoreChIndex = (Icu_17_TimerIp_ConfigPtr->ChannelMapping[Channel] & \
      ICU_CHNL_MAP_CHNL_INDEX_MASK);
#else
          ...
          ICU_CHNL_ACTIVITY_BIT_LENGTH))
  {
    ...
    else
    {
          ...
          (uint32)ICU_17_TIMERIP_LOW_TIME))
      {
        ...
        Is overflow flag set or the flag to indicate overflow of active phase is set?
        [/cover] */
    ...
    const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  CfgEdge =  (uint32)(Mcal_GetBitAtomic(*GlbIndexStatusPtr,\
         ICU_CHNL_EDGE_POS,ICU_CHNL_EDGE_BIT_LENGTH));
  /* Upper Nibble of AssignedHwUnitNumber holds the Channel Number */
  CcChannelNumber = ((uint8)(CellUsed & ICU_CCU_GET_UPPER_NIBBLE) >> \
      ICU_BRING_TO_BIT_0);
  /* Assign SR and R register base addresses */
  CcuSrRegPtr = &(Ccu6RegPtr->CC6SR[0]);
  CcuRRegPtr = &(Ccu6RegPtr->CC6R[0]);
...

  {
    ...
    switch(SigKind)
    {
      ...
      CcuActiveTime=(uint16)\
            Icu_lCcuSubtractWithOverflow(CcuDutyRegVal,
                          (uint16)ChannelDataPtr->SigMeasureParasCcu6.RegValue);
      CcuPeriod=(uint16)Icu_lCcuSubtractWithOverflow(
          CcuPeriodRegVal,(uint16)ChannelDataPtr->SigMeasureParasCcu6.RegValue);
      ChannelDataPtr->SigMeasureParasCcu6.ActiveTime = \
          (uint32)((CcuActiveTime << ICU_CCU6_ACTIVETIME_OFFSET) | (CcuPeriod));
  ...
  uint32 * const GlbIndexStatusPtr)
{
  ...
  if (ICU_17_TIMERIP_MODE_SLEEP == Icu_17_TimerIp_Varlist[CoreId]->GlobalMode)
  {
    ...
    /* Wakeup event is set in Icu_17_TimerIp_EnableWakeup and a check is needed to
...
**                                                                            **
** Syntax :           LOCAL_INLINE void Icu_lGtmTimeStamp_Isr(                **
**                    const uint8 CoreId                                      **
**              const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr **
**              const uint32 *const GlbIndexStatusPtr)                        **
**                                                                            **
