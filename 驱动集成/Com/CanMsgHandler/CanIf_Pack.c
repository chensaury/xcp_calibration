														  ...
														  uint8_t frame_length)        /**Rang:1~FRAME_MAX_DLC**/
{
	...
	can_signal.Signal_Value = Signal_Value;
	...
	{	can_signal_mask = 0;
		for(i = 0;i < signal_length;i++)  /**Trimming**/
		{
			can_signal_mask |= 0x1 << i;
		}
		...
		if(signal_byte_order == SIGNAL_BYTE_ORDER_INTEL)
		{
			...
			else
			{
				...
				{
					can_signal_mask = 0;
					for(i = 0;i < low_data_byte_position;i++)  /**Generate signal mask according to signal length**/
					{
						can_signal_mask |= 0xFF >> i;
					}
					data_buff[low_data_byte] &=(can_signal_mask);
				...
				{					
					for(byte_num = low_data_byte;byte_num <= high_data_byte;byte_num++)
					{
						...
						else
						{
							...
							can_signal_mask = 0;
							for(i = 0;i < shift;i++)  /**Generate signal mask according to signal length**/
							{
								can_signal_mask |= 0xFF << i;
							}
							data_buff[byte_num] &=(can_signal_mask);
		...
		else  /**signal_byte_order == SIGNAL_BYTE_ORDER_MOTOROLA**/
		{
			...
			else
			{
				...
				{				
					can_signal_mask = 0;
					for(i = 0;i < high_data_byte_position;i++)  /**Generate signal mask according to signal length**/
					{
						can_signal_mask |= 0xFF >> i;
					}
