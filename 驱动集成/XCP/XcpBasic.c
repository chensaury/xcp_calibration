...
//    }
  ...
  {//20201125下面这一行影响了太多，会影响进入除链接函数外的其他函数的状态，需要看看以前写的的函数是怎么运行的
   // xcp.SendStatus |= (vuint8)XCP_CRM_PENDING; //先暂时屏蔽，因为他影响到后面的返回函数了
    ApplXcpSend(xcp.CrmLen,&xcp.Crm.b[0]);//原版的发送202011117感觉没必要呢
   // ApplXcpSend(&xcp.Crm.b[0]);//为什么用这个不行呢，这个不应该才是好使的么，只用了一个形参啊20201117
  }                              //这里可能是在对应的.h文件里面有个extern的函数决定的所以用这个函数不好使可以改改试试
                                  //好几个都是用的两个参数，建议改can的驱动吧
...
static unsigned long long daqcount=0;
static unsigned long long odtcount=0;
static unsigned long long sendcount1=0;
static unsigned long long sendcount2=0;
...
vuint8 XcpEvent( vuint8 event )
{
  ...
  /* Data Stimulation (STIM) */
      ...
      for (odt=DaqListFirstOdt(daq);odt<=DaqListLastOdt(daq);odt++)
      {
        ...
        ApplXcpInterruptDisable(); /* The following codis not reentrant */
...
void XcpCommand( const vuint32* pCommand )
{
 ...
 /* XCP Data Stimulation Handler */
  ...
  else /* CC_CONNECT */
  {
    ...
    {
      /* Ignore commands if the previous command sequence has not been completed */   //20201124屏蔽了要不进不了下面的命令
#if defined ( XCP_ENABLE_SEND_QUEUE )
      ...
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_CRC32 )
        ...
        ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD44 )        
          ...
          case CC_GET_DAQ_PROCESSOR_INFO: /* PRQA S 2003 */ /* MD_Xcp_2003 */
    ...
    #endif
              CRM_GET_DAQ_PROCESSOR_INFO_PROPERTIES =//0X57;
            		  (vuint8)( DAQ_PROPERTY_CONFIG_TYPE
    #if defined ( XCP_ENABLE_DAQ_TIMESTAMP )
            ...
            case CC_GET_DAQ_RESOLUTION_INFO: /* PRQA S 2003 */ /*  */
              {
    #if defined ( XCP_ENABLE_TESTMODE )
                if ( gDebugLevel != 0) 
                {
                ...
                CRM_GET_DAQ_RESOLUTION_INFO_MAX_SIZE_DAQ  = 6;//(vuint8)XCP_MAX_ODT_ENTRY_SIZE;
                CRM_GET_DAQ_RESOLUTION_INFO_MAX_SIZE_STIM = 6;//(vuint8)XCP_MAX_ODT_ENTRY_SIZE;
    #if defined ( XCP_ENABLE_DAQ_TIMESTAMP )
    ...
    #else
                //CRM_GET_DAQ_RESOLUTION_INFO_TIMESTAMP_MODE =0x44;
                xcp.Crm.b[5]=0x44;
  ...
  #if defined ( XCP_ENABLE_DAQ_EVENT_INFO ) && defined ( kXcpMaxEvent )  //BY TAN 2021.09.01
            case CC_GET_DAQ_EVENT_INFO: /* PRQA S 2003 */ /* MD_Xcp_2003 */
              {
                ...
                CRM_GET_DAQ_EVENT_INFO_PROPERTIES = 0X0C;//kXcpEventDirection[event];
                CRM_GET_DAQ_EVENT_INFO_MAX_DAQ_LIST = 1; /* Only one DAQ-List available per event channel */
                CRM_GET_DAQ_EVENT_INFO_NAME_LENGTH = 0X0F;//kXcpEventNameLength[event];
                CRM_GET_DAQ_EVENT_INFO_TIME_CYCLE = 0X0A;//kXcpEventCycle[event];
    #if defined ( XCP_ENABLE_CANAPE_5_5_X_SUPPORT )
                CRM_GET_DAQ_EVENT_INFO_TIME_UNIT = kXcpEventUnit[event];
    #else
                CRM_GET_DAQ_EVENT_INFO_TIME_UNIT =0X00;// (vuint8)(kXcpEventUnit[event]>>4); /* ESCAN00090639 */
    #endif
                CRM_GET_DAQ_EVENT_INFO_PRIORITY =0X00;// 0;     /* Event channel prioritization is not supported. */
//                XcpSetMta( ApplXcpGetPointer( 0xFF, (vuint32)kXcpEventName[event]), 0xFF ); /* PRQA S 0306 */ /* MD_Xcp_0306 */

                XcpSetMta( ApplXcpGetPointer( 0xFF, (vuint32)0xd0000000), 0xFF );
    #if defined ( XCP_ENABLE_TESTMODE )
                if ( gDebugLevel != 0)
                {
...
//              {
//                error(CRC_DAQ_CONDIF) /* PRQA S 2001 */ /* MD_Xcp_2001 */      //BY TAN2021.07.11
//              }
...
#else
 // #error "Please define XCP_CPUTYPE_LITTLEENDIAN or XCP_CPUTYPE_BIGENDIAN."    20201117
#endif
